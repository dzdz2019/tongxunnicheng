...
import tongxun.zhy.dz.com.tongxun.thread.Task;
import tongxun.zhy.dz.com.tongxun.thread.ThreadWorker;
import tongxun.zhy.dz.com.tongxun.tools.anno.Inject;
import tongxun.zhy.dz.com.tongxun.tools.anno.InjectAssert;
import tongxun.zhy.dz.com.tongxun.tools.anno.InjectExtra;
import tongxun.zhy.dz.com.tongxun.tools.anno.InjectResource;
import tongxun.zhy.dz.com.tongxun.tools.anno.InjectView;
...
public class InjectUtil {
	...
	private static void getResource(Object obj, Field field, InjectResource resource) {
		Resources res = IocContainer.getIocContainer().getApplication().getResources();
		Object value = null;
		if (resource.color() != 0) {
	...
	public static void indectView(Object obj, Field field, InjectView injectview) {
		...
		// layout中获取
		if (layout != 0) {
			view = LayoutInflater.from(IocContainer.getIocContainer().getApplicationContext()).inflate(layout, null);
		} else {
			// 在其他view中的view
			...
			if (!TextUtils.isEmpty(inView)) {
				...
				if (viewId == 0) {
					...
					if (!TextUtils.isEmpty(idStr)) {
						try {
							String packageName = IocContainer.getIocContainer().getApplicationContext().getPackageName();
							Resources resources = IocContainer.getIocContainer().getApplicationContext().getPackageManager().getResourcesForApplication(packageName);
							viewId = resources.getIdentifier(idStr, "id", packageName);
							if (viewId == 0)
								throw new RuntimeException(idStr + "idStr或者id都未设置");
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
				...
				if (obj instanceof Activity) {
					...
					view = act.findViewById(viewId);
				} else if (obj instanceof Dialog) {
					Dialog act = (Dialog) obj;
					view = act.findViewById(viewId);
				} else if (obj instanceof Fragment) {
					Fragment act = (Fragment) obj;
					view = act.getView().findViewById(viewId);
				} else if (obj instanceof View) {
					View vtemp = (View) obj;
					view = vtemp.findViewById(viewId);
				} else if (obj instanceof ContentView) {
					ContentView vtemp = (ContentView) obj;
					view = vtemp.getContentView().findViewById(viewId);
				}
			}
		}
	...
	public static void injectStand(Object obj, Field field, Inject inject) {
		try {
			...
			if (!TextUtils.isEmpty(name)) {
				value = IocContainer.getIocContainer().get(name);

			} else {
				Class clazz = field.getType();
				String tag = inject.tag();
				if (!TextUtils.isEmpty(tag)) {
					value = IocContainer.getIocContainer().get(clazz, tag);
				} else {
					value = IocContainer.getIocContainer().get(clazz);
				}
			}
			field.set(obj, value);
		} catch (IllegalArgumentException e) {
	 ...
	 * @param as
	 */
	private static void getAssert(final Object activity, final Field field, final InjectAssert as) {
		AssetManager manager = IocContainer.getIocContainer().getApplication().getAssets();
		Class clazz = field.getType();
		Object value = null;
		try {
	...
	private static void setItemLongClickListener(Object activity, Field field, String itemClickMethod) {
		try {
			Object obj = field.get(activity);
			if (obj instanceof AdapterView) {
				((AdapterView) obj).setOnItemLongClickListener(new EventListener(activity).itemLongClick(itemClickMethod));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static boolean isCommonClazz(Class<?> clazz) {
		...
		boolean isok = pkg.startsWith("com.dm") || pkg.startsWith(IocContainer.getIocContainer().getApplicationContext().getPackageName());
		if (isok)
			return true;
		if (Consts.ioc_instal_pkg != null) {
			for (int i = 0; i < Consts.ioc_instal_pkg.length; i++) {
				if (pkg.startsWith(Consts.ioc_instal_pkg[i]))
					return true;
			}
		}
		return false;
